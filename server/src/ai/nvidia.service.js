  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!parsed.projectName || !parsed.theme || !Array.isArray(parsed.components)) {
        throw new Error('Invalid response structure from NVIDIA planning');
      }

      console.log('[NVIDIA] Successfully generated website plan');
      return parsed;
    } catch (error) {
      console.error(`NVIDIA Planning Error (Attempt ${attempt}/${maxRetries}):`, error.message);
      ...
      if (error?.status === 429 && attempt < maxRetries) {
        ...
        console.warn(`⚠️ Rate limited by NVIDIA. Waiting ${waitMs}ms before retrying...`);
      ...
      if (attempt === maxRetries) {
        throw new Error('Failed to generate website plan with NVIDIA. Please try again later.');
      }
    }
  }
};
...
9. Return ONLY raw TSX code. Start with 'import React'. Do not wrap in markdown \`\`\` tags.`;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Safety fallback: Strip markdown if NVIDIA accidentally includes it
      ...
      console.error(`NVIDIA Code Generation Error for ${componentName} (Attempt ${attempt}/${maxRetries}):`, error.message);
      ...
      if (error?.status === 429) {
        ...
        throw new Error(`AI rate limit reached. Please wait ${waitMins} minutes and try again. (NVIDIA API limit)`);
      }
      
      if (attempt === maxRetries) {
        throw new Error(`Failed to generate code for ${componentName} with NVIDIA.`);
      }
    }
  }
};
